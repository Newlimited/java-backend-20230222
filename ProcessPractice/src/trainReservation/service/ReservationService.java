...
public class ReservationService {
	...
	public ReservationInfo postReservation(PostReservationDto postReservationDto, GetTrainListDto getTrainListDto) {
		...
		if (!designationState) {
			System.out.println("좌석배정에 실패 했습니다.");
			return null;
		}
		...
		for (Cost cost : costs) {
			...
			boolean isEqualArraivalStation = 
					getTrainListDto.isEqualArrivalStation(cost.getArrivalStation());
			if (!isEqualDepartureStation || !isEqualArraivalStation)
				continue;
		...
		ReservationInfo reservationInfo = new ReservationInfo(
				...
				getTrainListDto.getDepartureStation(),departureTime,
				getTrainListDto.getArrivalStation(),arrivalTime,
				totalCost);
