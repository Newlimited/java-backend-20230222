...
//					코드의 중복을 제거할 수 있고 수정이 발생하면 
//					하나의 코드만 수정하면된다.
// 3. 프로그램 구조화 : 작업 단위로 여러 개의 메서드에 담아 프로그램을 구조화 할 수 있음

// 파라미터가 있는 메서드는 반드시 파라미터 범위 등과 같은 검증을 먼저 해줘야함
class HigherMathematics {
	static final double PIE = 3.14; // 전역변수 - > 클래스 내에서 전부다 사용할 수 있게 함.
	
	// 클래스 변수와 마찬가지로 메서드에 static 포함하게 되면
	// 클래스 메서드로 사용할 수 있음{클래스로 사용 할 수 있음}
...
//		final double PIE = 3.14; // 지역변수 -> 해당 메서드 내에서만 사용 가능
...
//		HigherMathematics math = new HigherMathematics(); static double 로 해주면 생성자가 필요없다.
		double size = HigherMathematics.getTriangleSize(10.0, 10.0);// math 대신바로 ...
